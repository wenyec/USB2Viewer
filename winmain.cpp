...
// the change for working branch test
//
//////////////////////////////////////////////////////////////////////////
...
struct _capstuff
{
	...
	// for rotatation implementation in capture pin
	IBaseFilter	*pvidRotate;
	IVideoRotation *pvidRotater;

	// for still pin
	IBaseFilter *pSampleGrabberFilterStill;
	ISampleGrabber *pSampleGrabberStill;
	IBaseFilter *pNullRendererStill;
	// for rotatation implementation in still pin
	IBaseFilter	*pvidRotateStill;
	IVideoRotation *pvidRotaterStill;
	BOOL isStillSup;  // the flag for still capture support --wenye
	DWORD stillWidth;
	DWORD stillHeight;
	StillFormats stillFmts[16];

	int FlashT = 200;  //default flash setting 
	int Rotate_D = 0;   //default rotate value
} gcap;
...
const WCHAR WINDOW_NAME[] = L"Videology USB 3.0 ViewerPlus";
...
#if 0
		/* the still image rotation */
		BYTE *ppostBuffer = new BYTE [BufferLen];

		// Write the file header.
		BITMAPINFOHEADER *pBitmapInfoHeader = HEADER(pVideoHeader);
		LONG biWidth, biHeight;
		switch (gcap.Rotate_D){
		case 0:
			biWidth = pBitmapInfoHeader->biWidth;
			biHeight = pBitmapInfoHeader->biHeight;
			pBitmapInfoHeader->biHeight = -pBitmapInfoHeader->biHeight;
			for (int j = 0; j < biWidth; j++)
			{
				for (int i = 0; i < biHeight; i++)
				{
					*(ppostBuffer + (j*biWidth) + i) = *(pBuffer + (j*biWidth) + i);
				}
			}
			break;
		case 1:
			biWidth = pBitmapInfoHeader->biHeight;
			biHeight = pBitmapInfoHeader->biWidth;
			for (int j = 0; j < biHeight*3; j++)
			{
				for (int i = 0; i < biWidth; i++)
				{
					*(ppostBuffer + (j*biWidth) + i) = *(pBuffer + (biWidth - i)*biHeight + j);
				}
			}
			break;
		case 2:
			...
			for (int j = 0; j < biHeight; j++)
			{
				for (int i = 0; i < biWidth; i++)
				{
					*(ppostBuffer + (j*biWidth) + i) = *(pBuffer + ((biHeight - j)*biWidth) + biWidth - i);
				}
			}
			break;
		case 3:
			biWidth = pBitmapInfoHeader->biHeight;
			biHeight = pBitmapInfoHeader->biWidth;
			for (int j = 0; j < pBitmapInfoHeader->biHeight; j++)
			{
				for (int i = 0; i < biHeight; i++)
				{
					*(ppostBuffer + (j*biWidth) + i) = *(pBuffer + (i*biHeight) + biHeight - j);
				}
			}
			break;
		}
		pBitmapInfoHeader->biWidth = biWidth;
		//pBitmapInfoHeader->biHeight = biHeight;
#endif
		DWORD dwWritten = 0;
		WriteFile(hf, &bfh, sizeof(bfh), &dwWritten, NULL);
		WriteFile(hf, HEADER(pVideoHeader), cbBitmapInfoSize, &dwWritten, NULL);
		//WriteFile(hf, pBitmapInfoHeader, cbBitmapInfoSize, &dwWritten, NULL);
		WriteFile(hf, pBuffer, BufferLen, &dwWritten, NULL);
		//WriteFile(hf, ppostBuffer, BufferLen, &dwWritten, NULL);
		CloseHandle(hf);
//		delete[] ppostBuffer;
		return S_OK;
	}
...
#define DESCSIZE 80
	...
	if (gcap.pmVideo != pmVideo || gcap.pmAudio != pmAudio)
	{
		...
		if (gcap.fCaptureGraphBuilt || gcap.fPreviewGraphBuilt)
			TearDownGraph();
		if (gcap.pvidRotate){
			SAFE_RELEASE(gcap.pvidRotater);
			SAFE_RELEASE(gcap.pvidRotate);
			gcap.pFg->RemoveFilter(gcap.pvidRotate);
		}
		if (gcap.pvidRotateStill){
			SAFE_RELEASE(gcap.pvidRotaterStill);
			SAFE_RELEASE(gcap.pvidRotateStill);
			gcap.pFg->RemoveFilter(gcap.pvidRotate);
		}
...
void OnClose(HWND /*hwnd*/)
{
	...
	if (gcap.pvidRotate){
		SAFE_RELEASE(gcap.pvidRotater);
		SAFE_RELEASE(gcap.pvidRotate);
		hr = gcap.pFg->RemoveFilter(gcap.pvidRotate);
	}
	if (gcap.pvidRotateStill){
		SAFE_RELEASE(gcap.pvidRotaterStill);
		SAFE_RELEASE(gcap.pvidRotateStill);
...
void OnSize(/*HWND hwnd,*/ WPARAM wParam, LPARAM lParam/*, UINT state*/)
{
	...
	if (gcap.Rotate_D == 0 || gcap.Rotate_D == 2){
		lWidth = HEADER(pmt->pbFormat)->biWidth;
		lHeight = ABS(HEADER(pmt->pbFormat)->biHeight);
	}
	else{
		lHeight = HEADER(pmt->pbFormat)->biWidth;
		lWidth = ABS(HEADER(pmt->pbFormat)->biHeight);
	}
	aspRetio = (float)lWidth / (float)lHeight;
	DeleteMediaType(pmt);
...
void Rotation(int Rotate_D){

	StopPreview();
	TearDownGraph();

	gcap.Rotate_D = Rotate_D;

	//CComQIPtr<IVideoRotation> m_MainRotate;
	//m_MainRotate = gcap.pvidRotate;
	//m_MainRotate->SetRotation(gcap.Rotate_D);
	//m_MainRotate->Release;
	gcap.pvidRotater->SetRotation(gcap.Rotate_D);
	gcap.pvidRotaterStill->SetRotation(gcap.Rotate_D);

	gcap.pFg->AddFilter(gcap.pvidRotate, L"Rotate Filter");
	gcap.pFg->AddFilter(gcap.pvidRotateStill, L"StillRotate Filter");
...
#if 0
	if (1&&(gcap.Rotate_D == 1 || gcap.Rotate_D == 3)){
		aspRetio = (float)(image_h) / (float)(image_w);
		//ResizeWindow(image_h, image_w);
	}
	else{
		aspRetio = (float)(image_w) / (float)(image_h);
		//ResizeWindow(image_w, image_h);
	}
#endif
...
void OnCommand(HWND hwnd, int id, HWND /*hwndCtl*/, UINT /*codeNotify*/)
{
	...
	case ID_5MPCAMERASETTINGS_GAMMAAND2DNR:
		...
		break;
	case ID_13MPCOLORCAMERA_0ROTATATION:
		Rotation(rotate0);
		break;
	case ID_13MPCOLORCAMERA_90ROTATION:
		Rotation(rotate90);
		break;
	case ID_13MPCOLORCAMERA_180DEGREEROTATION:
		Rotation(rotate180);
		break;
	case ID_13MPCOLORCAMERA_270DEGREEROTATION:
		Rotation(rotate270);
		break;
	...
	*/
	...
	if (gcap.pVSC && gcap.pVSC->GetFormat(&pmt) == S_OK)
	{
		...
		if (pmt->formattype == FORMAT_VideoInfo)
		{
			...
			if (gcap.Rotate_D == 1 || gcap.Rotate_D == 3){
				aspRetio = (float)(ABS(HEADER(pmt->pbFormat)->biHeight)) / (float)(HEADER(pmt->pbFormat)->biWidth);
				image_h = HEADER(pmt->pbFormat)->biWidth;
				image_w = ABS(HEADER(pmt->pbFormat)->biHeight);
				ResizeWindow(image_h, image_w);
			}
			else{
	...
	hr = gcap.pFg->AddFilter(gcap.pvidRotate, L"Rotate Filter");
	hr = gcap.pFg->AddFilter(gcap.pvidRotateStill, L"StillRotate Filter");
...
void    OnROISetting(HWND hwnd)
{
	...
	if (gcap.pVSC && gcap.pVSC->GetFormat(&pmt) == S_OK)
	{
		...
		{
			// resize our window to the default capture size
			if (0&&(gcap.Rotate_D == 1 || gcap.Rotate_D == 3)){
				ResizeWindow(ABS(HEADER(pmt->pbFormat)->biHeight), HEADER(pmt->pbFormat)->biWidth);
			}
			else{
...
HRESULT	OnSelectedFlashValu(HWND hwnd);
...
void OnInitNoiseReductionDialog(HWND hwnd)
{
...
#endif
	int startLvl = 0, endLvl = gcap.FlashT; //set defauld value
#if 0
	...
	//Edit_SetText(hListAGCLVL, L"0");
	EnableWindow(hListEdit2DNREndLVL, TRUE); //for Flash Pre Start Time Set.
...
HRESULT	OnSelectedFlashValu(HWND hwnd)
{
	...
	int num;
	HWND hListEdit2DNREndLVL = GetDlgItem(hwnd, IDC_EDIT_2D_NR_GAINEND_VAL);//
	num = Edit_GetText(hListEdit2DNREndLVL, getStr, count);//
	WideCharToMultiByte(CP_ACP, 0, getStr, -1, tempStr, num, &DefChar, NULL);
	tempStr[num+1] = '\0';
	gcap.FlashT = (long long)strtoll(tempStr, NULL, 10);
/*
	char str[4] = { '\0' };
	char DefChar = ' ';
	int xHexByte;
...
INT_PTR CALLBACK ImageResSetDlgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDOK:
			...
			BuildPreviewGraph();
			StartPreview();
			//ResizeWindow(gcap.stillWidth, gcap.stillHeight);//SendMessage(ghwndApp, WM_SIZE, SIZE_RESTORED, 0);
			EndDialog(hwnd, LOWORD(wParam));
			return TRUE;
#if 0
		case IDC_BTN_DIR:
...
CAtlFile m_Device;
WINUSB_INTERFACE_HANDLE m_Usb;
int FlsInterval = 0;
int IntervalWait = 0;
unsigned long SnapTime;


void Flash(WINUSB_INTERFACE_HANDLE m_Usb, bool State)
{
	...
	DWORD dwActual;
	WinUsb_ControlTransfer(m_Usb, wsp, NULL, 0, &dwActual, NULL);
}
...
void SetFlash()
{
	if (m_Device)
		return;
	CString  szPath = GetDevicePath(&GUID_V24ZFLASH);
	if (szPath)
	{
		m_Device.Create(
			szPath,
	...
	{
		MessageBox(0, L"24Z flash board was not found.", L"24Z Flash", MB_OK);
		return;
	}
	WinUsb_Initialize(m_Device, &m_Usb);
}

static DWORD WINAPI FlashThreadBridge(LPVOID Context)
{
	unsigned int interval = *((unsigned int*)Context);
	//Sleep(600);
	//Flash(m_Usb, true);//when turn off only
	Sleep(interval);
	Flash(m_Usb, false);
	return 0;
//	return ((PowerZoomHandler*)Context)->FlashThread();
}
/*
DWORD FlashThread()
{
...
void StartFlash(int iMilliseconds)
{
	IntervalWait = iMilliseconds;
...
void OnFlash()  // for flash control
{
#if 0
	if (m_Device){
		FlsInterval = 500;
		StartFlash(FlsInterval);
		SnapTime = GetTickCount() + FlsInterval;
		FlsInterval = 0;
	}
#else	
	// Connect up to the hardware.
	Flash(m_Usb, true);
#endif
}
...
BOOL InitCapFilters()
{
	...
	hr = CoCreateInstance(CLSID_RotateFilter, NULL, 
		CLSCTX_INPROC_SERVER, IID_IBaseFilter, 
		(void **)&gcap.pvidRotate); //create the video rotation filter
	hr = CoCreateInstance(CLSID_RotateFilter, NULL,
		CLSCTX_INPROC_SERVER, IID_IBaseFilter,
		(void **)&gcap.pvidRotateStill); //create the still image rotation filter
	hr = gcap.pvidRotate->QueryInterface(IID_Rotater, (void**)&gcap.pvidRotater); //query the video ratation filter interface
	hr = gcap.pvidRotate->QueryInterface(IID_Rotater, (void**)&gcap.pvidRotaterStill); //query the video ratation filter interface
	hr = gcap.pFg->AddFilter(gcap.pvidRotate, L"Rotate Filter");
	hr = gcap.pFg->AddFilter(gcap.pvidRotateStill, L"StillRotate Filter");

	//ConnectFilters(gcap.pFg, gcap.pVCap, gcap.pvidRotate);
	//ConnectFilters(gcap.pFg, gcap.pvidRotate, gcap.pSampleGrabberFilter);
	...
	{
		gcap.isStillSup = TRUE;
		gcap.stillWidth = 4128;
		gcap.stillHeight = 3096;
...
BOOL BuildPreviewGraph()
{
	...
	*/
	...
	//hr = ConnectFilters(gcap.pFg, gcap.pVCap, gcap.pvidRotate);
	//hr = ConnectFilters(gcap.pFg, gcap.pvidRotate, gcap.pSampleGrabberFilter); ////for the rotate
	/* before render stream take rotation of the stream and image */
	gcap.pvidRotater->SetRotation(gcap.Rotate_D);
	gcap.pvidRotaterStill->SetRotation(gcap.Rotate_D);
	int isIt640 = 0;
	if (gcap.pVSC)
	...
	else
	{
		...
		//SmartPtr<IVideoRotation> m_mainRotate;
		hr = gcap.pBuilder->RenderStream(&PIN_CATEGORY_PREVIEW,
			&MEDIATYPE_Video, gcap.pVCap, gcap.pvidRotate /*NULL*/, gcap.VideoRenderer);
		//gcap.pvidRotater->SetRotation(gcap.Rotate_D);
		//m_MainRotate->Release;
		...
		hr = gcap.pBuilder->RenderStream(&PIN_CATEGORY_STILL, &MEDIATYPE_Video, gcap.pVCap, gcap.pvidRotateStill, gcap.pSampleGrabberFilter/*NULL*/); // add --wenye
//		hr = gcap.pBuilder->RenderStream(&PIN_CATEGORY_STILL, &MEDIATYPE_Video, gcap.pvidRotateStill, gcap.pSampleGrabberFilter, gcap.pNullRendererStill); // add --wenye
		...
		if (gcap.pVSC && SUCCEEDED(gcap.pVSC->GetFormat(&pmtDV)))
		{
			if (pmtDV->formattype == FORMAT_DvInfo)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					{
						if (0&&(gcap.Rotate_D == 1 || gcap.Rotate_D == 3))
							ResizeWindow(abs(lHeight), lWidth);
						else
							ResizeWindow(lWidth, abs(lHeight));
					}
				}
